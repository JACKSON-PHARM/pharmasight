-- Full refresh of item_branch_snapshot for ALL branches (all items per branch).
-- Paste into Supabase SQL Editor and run. Loops over each (company_id, branch_id) and runs
-- the same bulk logic as the Python script. Increase statement_timeout if you hit limits.
--
-- Optional: to refresh only branches that have ledger data, change the loop source to:
--   SELECT DISTINCT company_id, branch_id FROM inventory_ledger

SET statement_timeout = '0';  -- no per-statement limit; each branch runs to completion
...
items_base AS (
  ...
  FROM items
  WHERE company_id = %1$L AND is_active = true
),
stock AS (
  SELECT item_id, COALESCE(current_stock, 0) AS current_stock
  FROM inventory_balances
  WHERE branch_id = %2$L AND company_id = %1$L
),
lpp AS (
...
wavg AS (
  ...
  FROM inventory_ledger
  WHERE branch_id = %2$L AND company_id = %1$L AND quantity_delta > 0
  GROUP BY item_id
),
costs AS (
...
BEGIN
  FOR r IN
    SELECT DISTINCT company_id, branch_id FROM item_branch_snapshot
    ORDER BY company_id, branch_id
  LOOP
    EXECUTE format(bulk_sql, r.company_id, r.branch_id);
    branch_count := branch_count + 1;
    RAISE NOTICE 'Refreshed branch % (company %, branch %)', branch_count, r.company_id, r.branch_id;
  END LOOP;
  RAISE NOTICE 'Done. Total branches refreshed: %', branch_count;
END $$;
